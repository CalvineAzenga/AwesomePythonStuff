...
from faceRecAPI import show_prediction_labels_on_image, markAttendance
...
mainBackImage = mainBackImage.resize((root.winfo_width(), root.winfo_height()), Image.ANTIALIAS)
...
startRecognitionImage = startRecognitionImage.resize((150, 150), Image.ANTIALIAS)
...
attendanceImage = attendanceImage.resize((150, 150), Image.ANTIALIAS)
...
signupImage = signupImage.resize((100, 50), Image.ANTIALIAS)
...
signupBtn = Button(bottom_canv, image=signupImage, compound=CENTER, border=0, relief="flat", background="#25507A",
                   cursor="hand2", command=dataCollectionPane.show)
    ...
    for file in os.listdir("./available"):
        try:
            ...
            if ret:
                ...
                try:
                    regNo = str("SC212/{}/2018".format(randint(1000, 1035)))
                    outcomes = conn.fillRecognitionData(regNo=regNo.upper())[0]
                    fillCard(outcomes[0], outcomes[1], outcomes[2], outcomes[3], outcomes[4], outcomes[5])
                    lblAttendeeName.configure(text=str(outcomes[0]))
                    markAttendance(regNo=regNo)
                except Exception as msg:
                    lblAttendeeName.configure(text="")
                    pass
                img = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                img = Image.fromarray(img)
                # img=img.resize((cameraCanvas.winfo_width(),cameraCanvas.winfo_height()),Image.ANTIALIAS)
...
startRecognitionBtn.configure(command=fromFolderThread)
...
def fillTreeview():
    conn = DbRetrieve()
    resultList = conn.getTreeviewData()
